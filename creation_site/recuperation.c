...
///Fonction qui active le site
void activation (char* name){
    char* cmd=malloc(100);
    sprintf(cmd,"sudo a2ensite %s",name);
    popen(cmd,"w");
    popen("sudo systemctl reload apache2","w");
   // printf("<meta http-equiv='refresh' content='0' url=\"http://www.anita.com/cgi-bin/creation_site/recuperation.cgi\">");
}
///Fonction qui desactive le site 
void desactivation(char* name){
    char* cmd = malloc(100);
...
///Fonction qui crée le fichier.conf
void creation_conf(){
    ...
    if(take!=NULL){
      ...
      // printf("%s<br>",withoutHex(documentRoot));
			...
			strcat(creation,".conf");  //le fichier .conf est créé dans le repertoire ou l'on se trouve
           
            sprintf(host,"127.0.0.1\t%s",serverName);
       //   printf("%s\n",host); //ce que l'on va mettre dans /etc/hosts
			...
			sprintf(cmd,"sudo cp %s /etc/apache2/sites-available/",creation); //copier le fichier .conf vers /etc/apache2/sites-available
	...
	free(serverName); free(nom_serveur);  free(name); free(admin);  free(documentRoot); free(racine); free(alias); free(dir1a); free(dir1b); free(dir1c);
	free(dir2a); free(dir2b); free(dir2c); free(dir2d);	free(err1a); free(err1b); free(virtual1a); free(virtual1b); free(creation); free(cmd); free(host);
